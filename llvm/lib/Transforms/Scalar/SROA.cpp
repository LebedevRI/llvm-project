...
public:
  ...
  struct CacheEntry {
    Value *ByteOffset = nullptr, *BitOffset = nullptr;
  };
  using PhonyOffsetCache = SmallDenseMap<GetElementPtrInst *, CacheEntry>;
...
getAddressCalculationStack(LoadInst *LI,
                           AllocaSlices::PhonyOffsetCache &GEPCache) {
  SmallVector<GetElementPtrInst *, 8> Stack;
  ...
  while (true) {
    ...
    case Instruction::GetElementPtr: {
      ...
      Stack.emplace_back(CurrGEP);
      // Did we previously deal with this GEP?
      if (!GEPCache.insert({CurrGEP, {}}).second)
        return Stack;  // We know it's accumulated byte offset. Done recursing.
...
getVariableBitOffsetIntoAlloca(LoadInst *LI,
  ...
  if (Value *BitOffset = GEPCache[GEPStack.front()].BitOffset)
    return BitOffset;
  ...
  while (!GEPStack.empty()) {
    ...
    if (!ByteOffset)
      ByteOffset = CurrByteOffset;
    else {
      assert(ByteOffset->getType() == CurrByteOffset->getType() &&
             "Index type changed?");
      ByteOffset =
          Builder.CreateAdd(ByteOffset, CurrByteOffset,
                            CurrGEP->getName() + ".byteoff", /*HasNUW=*/false,
                            /*HasNSW=*/CurrGEP->isInBounds());
    }
    CacheEntry.ByteOffset = ByteOffset;
  }

  // Finally, we know the byte offset, multiply by CHAR_BIT to get bit offset.
  Value *&BitOffset = GEPCache[CurrGEP].BitOffset;
  BitOffset =
      Builder.CreateMul(ByteOffset, ConstantInt::get(ByteOffset->getType(), 8),
                        ByteOffset->getName() + ".numbits", /*HasNUW=*/true,
                        /*HasNSW=*/true);
  return BitOffset;
}
